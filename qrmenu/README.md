...
- [x] **4. Корзина и заказ:** цена считается только на бэкенде, Idempotency-Key, статусы, правка позиций до «подан», лента заказов в админке в реальном времени (WebSocket + Redis) со звуком и уведомлениями
...
- ~3000 заказов за последние 90 дней на разных языках (с ростом, чтобы было видно «к прошлому периоду») — для «Аналитики»;
...
| PUT | `/api/admin/orders/{id}/items` | персонал; до «подан» (`pending`/`cooking`) |
...
- Статусы: `pending → cooking → served → closed`, `rejected` — из `pending` или `cooking`. «Принять» (`accepted`) одним нажатием переводит заказ сразу в `cooking`. Первый заказ сессии приходит в `pending`, если включено «подтверждение первого заказа», иначе сразу `cooking`. Статус `accepted` остаётся только у старых заказов.
- Гость видит статус своего заказа прямо на главном экране («Ждёт подтверждения» → «Готовится» → «Подано»), обновляется вживую; поданный заказ исчезает с главного экрана через 20 минут (в «Моих заказах» остаётся).
